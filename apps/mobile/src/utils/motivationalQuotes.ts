// Motivational Quotes with i18n support
...
// Quote definitions with translation keys
// The actual text is stored in locale files (tr.json, en.json, es.json, de.json)
...
/**
 * Get the quote index for a given date
 * @param dateKey - Server date in YYYY-MM-DD format
 */
export const getQuoteIndex = (dateKey: string): number => {
...
/**
 * Get the daily quote based on server date
 * @param dateKey - Date in YYYY-MM-DD format
 * @param t - Optional translation function from useTranslation hook
 * @returns Quote object with textKey (or translated text if t is provided)
 */
export const getDailyQuote = (dateKey: string, t?: (key: string) => string): Quote => {
    const year = parseInt(dateKey.split('-')[0]);
    const month = parseInt(dateKey.split('-')[1]) - 1; // JavaScript months are 0-indexed
...
/**
 * Get random quote
 * @returns Quote object with textKey
 */
